...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-title { font-size: 1.3rem; font-weight: 600; margin-bottom: 25px; display: flex; align-items: center; gap: 10px; }
        ...
        @media (max-width: 768px) { .sidebar { transform: translateX(-100%); } .sidebar.active { transform: translateX(0); } .main-content { margin-left: 0; } .form-grid { grid-template-columns: 1fr; } }
...
<body>
    <div class="dashboard-container">
        ...
        <main class="main-content" id="mainContent">
            ...
            <div class="form-container">
                <div class="form-title"><i class="fas fa-user-plus"></i> Student Registration Form</div>
                <form id="addStudentForm" method="POST" action="../../../api/students.php?action=add">
                    <div class="form-grid">
                        ...
                        <div class="form-group"><label>Roll Number *</label><input type="text" class="form-control" name="roll_number" required></div>
                        <div class="form-group"><label>Enrollment Number *</label><input type="text" class="form-control" name="enrollment_number" required></div>
                        <div class="form-group"><label>Course *</label><input type="text" class="form-control" name="course" value="BSC IT" required></div>
                        <div class="form-group"><label>Semester *</label><input type="number" class="form-control" name="semester" value="3" required></div>
                        <div class="form-group"><label>Batch *</label><input type="text" class="form-control" name="batch" value="2023-2026" required></div>
                        <div class="form-group"><label>Father's Name</label><input type="text" class="form-control" name="father_name"></div>
                        <div class="form-group"><label>Mother's Name</label><input type="text" class="form-control" name="mother_name"></div>
                        ...
                        <div class="form-group"><label>Admission Date</label><input type="date" class="form-control" name="admission_date"></div>
    ...
    <script>
        document.getElementById('addStudentForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('../../../api/students.php?action=add', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
                const result = await response.json();
                if(result.success) { showNotification('Student added successfully!', 'success'); setTimeout(() => window.location.href = 'students-list.php', 1500); } 
                else { showNotification(result.message, 'error'); }
            } catch(error) { showNotification('Error adding student', 'error'); }
        });
